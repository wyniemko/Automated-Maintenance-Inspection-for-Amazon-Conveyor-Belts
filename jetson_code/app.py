...
benchmark_speed = 12.0
benchmark_length = 15.0

@app.route('/update_benchmark_values', methods=['POST'])
def update_benchmark_values():
    global benchmark_straightness_min, benchmark_straightness_max, benchmark_rips_max, benchmark_vertices_min, benchmark_vertices_max, benchmark_speed, benchmark_length
    ...
    benchmark_speed = float(request.form['benchmark_speed'])
    benchmark_length = float(request.form['benchmark_length'])
...
# Function to trigger the /process, /results, and /data routes at 10am
def trigger_process_results_data_routes():
    # You can set default values for speed and length here
    default_speed = benchmark_speed
    default_length = benchmark_length
    # Call the process, results, and data functions to trigger the routes
    with app.app_context():
        process(default_speed, default_length)
        results()
        data()

# Initialize the APScheduler
scheduler = BackgroundScheduler()
scheduler.add_job(func=trigger_process_results_data_routes, trigger='cron', hour=10)
scheduler.start()
...
def index():
    ...
    return render_template('index.html', benchmark_straightness_min=benchmark_straightness_min, benchmark_straightness_max=benchmark_straightness_max, benchmark_rips_max=benchmark_rips_max, benchmark_vertices_min=benchmark_vertices_min, benchmark_vertices_max=benchmark_vertices_max, speed=speed, length=length, benchmark_speed=benchmark_speed, benchmark_length=benchmark_length)
...
def process():
    ...
    for frame_num in range(total_frames):
        ret, frame = cap.read()
        if ret:
            # To save the entire frame without cropping, use the line below
...
def results():
    ...
    for filename in os.listdir("Edge"):
        ...
        # Initialize the total deviation
        total_deviation = 0
        ...
        if lines is not None:
            for line in lines:
                ...
                # Calculate deviation from ideal vertical line (pi/2 or 3*pi/2)
                deviation = min(abs(theta - np.pi / 2), abs(theta - 3 * np.pi / 2))
                total_deviation += deviation

            # Calculate the average deviation from the ideal straight lines
            avg_deviation = total_deviation / len(lines)

            # Convert the average deviation to a straightness value between 0 and 100
            straightness = 100 - (avg_deviation / (np.pi / 2) * 100)
        else:
            straightness = 0
    ...
    # Part 4: In each frame, determine if there is any presence of rips
    ...
    # Part 5: List all of the Rips, from largest to smallest.
    ...
    # Part 6: Calculate the average overall surface condition of the belt
    num_good_frames = 0
    avg_num_vertices = 0
    avg_solidity = 0
    ...
    c.execute('''INSERT INTO results
                (avg_straightness, blue_values, avg_num_vertices, avg_solidity, belt_speed, belt_length, date, time)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                (avg_straightness, str(blue_values), avg_num_vertices, avg_solidity, benchmark_speed, benchmark_length, current_date, current_time))
